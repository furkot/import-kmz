...
const debug = Debug('furkot-import-kmz');
 ...
 */
export async function findKml(blob) {
...
async function centralDirectory(blob, { cdStart, cdLen, cdCount }) {
  ...
  async function findKml() {
    for (let entry = entryAt(0), i = 0; i < cdCount; entry = entry.next(), i++) {
  ...
  function entryAt(index) {
    ...
    function size() {
      return (
        CENTRAL_DIRECTORY_FILE_HEADER_LEN +
        nameLen() +
